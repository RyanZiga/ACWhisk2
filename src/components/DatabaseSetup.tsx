...
import { SignUpDebugger } from './SignUpDebugger'
...
export function DatabaseSetup() {
  ...
  const [tables, setTables] = useState<TableStatus[]>([
    ...
    { name: 'portfolios', exists: false, required: true, description: 'Student portfolios' },
    { name: 'portfolio_items', exists: false, required: true, description: 'Portfolio content items' },
    { name: 'user_skills', exists: false, required: true, description: 'User culinary skills tracking' },
    { name: 'user_achievements', exists: false, required: true, description: 'User achievements and certifications' },
...
CREATE POLICY "Users can view portfolio items from public portfolios" ON portfolio_items FOR SELECT USING (
  EXISTS (SELECT 1 FROM portfolios p WHERE p.id = portfolio_items.portfolio_id AND p.is_public = true)
);
CREATE POLICY "Users can view own portfolio items" ON portfolio_items FOR SELECT USING (
  EXISTS (SELECT 1 FROM portfolios p WHERE p.id = portfolio_items.portfolio_id AND p.user_id = auth.uid())
);
CREATE POLICY "Users can create portfolio items for own portfolios" ON portfolio_items FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM portfolios p WHERE p.id = portfolio_items.portfolio_id AND p.user_id = auth.uid())
);
CREATE POLICY "Users can update own portfolio items" ON portfolio_items FOR UPDATE USING (
  EXISTS (SELECT 1 FROM portfolios p WHERE p.id = portfolio_items.portfolio_id AND p.user_id = auth.uid())
);
CREATE POLICY "Users can delete own portfolio items" ON portfolio_items FOR DELETE USING (
  EXISTS (SELECT 1 FROM portfolios p WHERE p.id = portfolio_items.portfolio_id AND p.user_id = auth.uid())
);

-- User skills policies
CREATE POLICY "Anyone can view user skills" ON user_skills FOR SELECT USING (true);
CREATE POLICY "Users can create own skills" ON user_skills FOR INSERT WITH CHECK (auth.uid() = user_id);
...
`
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Priority: Simple Database Checker - Works without any database functions */}
      <MotionCard 
        className="glass-card"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
